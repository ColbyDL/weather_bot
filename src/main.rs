...
async fn distance(
    ...
    match get_weather(city1).await {
        Ok(weather_response) => {
            ...
            let _response = format!("The weather in {}, lat {}, lon {}", city1, coord1, coord2);
            ...
            match get_weather(city2).await {
                ...
                Err(_) => {
                    ctx.say(format!("Could not find weather data for '{}'", city2))
                        .await?;
                }
            }
        }
        Err(_) => {
            ctx.say(format!("Could not find weather data for '{}'", city1))
                .await?;
        }
    }
...
async fn main(
    ...
    let client = ClientBuilder::new(secret, intents)
        .framework(framework)
        .await
